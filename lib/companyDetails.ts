...
export const getSubsidyForCapacity = (capacityKw: number): { central: number; state: number; total: number } => {
    let central = 0;
    let state = 0;

    // Subsidy is fixed in tiers, not dynamically multiplied by decimals.
    if (capacityKw >= 3) {
        central = 78000;
        state = 30000;
